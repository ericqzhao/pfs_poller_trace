...
void _spdk_poller_trace_record(uint64_t tsc, uint16_t tpoint_id, uint16_t poller_id,
			uint32_t size, uint64_t object_id, int num_args, ...);
...
#define _spdk_poller_trace_record_tsc(tsc, tpoint_id, poller_id, size, object_id, num_args, ...)	\
	do {											\
		assert(tpoint_id < SPDK_TRACE_MAX_TPOINT_ID);					\
		if (g_trace_histories == NULL ||						\
		    !((1ULL << (tpoint_id & 0x3F)) &						\
		...
		_spdk_poller_trace_record(tsc, tpoint_id, poller_id, size, object_id,			\
				   num_args, ## __VA_ARGS__);					\
	} while (0)
...
#define spdk_poller_trace_record_tsc(tsc, tpoint_id, poller_id, size, object_id, ...)	\
	_spdk_poller_trace_record_tsc(tsc, tpoint_id, poller_id, size, object_id,	\
			       spdk_trace_num_args(__VA_ARGS__), ## __VA_ARGS__)
/**
 * Record the current trace state for tracing tpoints. Debug tool can read the
 * information from shared memory to post-process the tpoint entries and display
...
#define spdk_poller_trace_record(tpoint_id, poller_id, size, object_id, ...) \
	spdk_poller_trace_record_tsc(0, tpoint_id, poller_id, size, object_id, ## __VA_ARGS__)
/**
...
int spdk_poller_trace_init(const char *shm_name, uint64_t num_entries, int num_pollers);
