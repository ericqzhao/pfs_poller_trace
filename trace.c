...
void
_spdk_poller_trace_record(uint64_t tsc, uint16_t tpoint_id, uint16_t poller_id, uint32_t size,
                   uint64_t object_id, int num_args, ...)
{
        struct spdk_trace_history *lcore_history;
        struct spdk_trace_entry *next_entry;
...
int
spdk_poller_trace_init(const char *shm_name, uint64_t num_entries, int num_pollers)
{
        uint32_t i = 0;
        int histories_size;
        uint64_t lcore_offsets[SPDK_TRACE_MAX_LCORE + 1] = { 0 };
